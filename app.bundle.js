/* Helpers — Lemon Squeezy license (variants 1314978 / 1314998 / 1315002). */
...
/** Lemon Squeezy storefront checkout (license overlay “Purchase License”). */
const HELPERS_LICENSE_PURCHASE_URL = 'https://helperstudio.lemonsqueezy.com/';
    ...
    xhr.onreadystatechange = function () {
        ...
        try {
            ...
            var tierName = (meta.variant_name && meta.variant_name.trim()) ? meta.variant_name.trim()
                : (meta.variant_id != null && LEMON_VARIANT_NAMES[meta.variant_id]) ? LEMON_VARIANT_NAMES[meta.variant_id] : 'Helpers';
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (purchaseLink) {
        purchaseLink.setAttribute('data-external', HELPERS_LICENSE_PURCHASE_URL);
        purchaseLink.addEventListener('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            if (typeof window.openExternalUrl === 'function') {
                window.openExternalUrl(HELPERS_LICENSE_PURCHASE_URL);
            }
        });
    }
...
/**
 * Panel utilities: [data-external] → browser; help mailto → clipboard + feedback (merged from helpers-utils.js).
 */
(function (global) {
  'use strict';
  ...
  function openExternalUrl(url) {
    ...
    try {
      var cepUtil = global.cep && global.cep.util;
      if (cepUtil && typeof cepUtil.openURLInDefaultBrowser === 'function') {
        return cepUtil.openURLInDefaultBrowser(resolved);
      }
    } catch (e0) { /* fall through */ }
    try {
      var CSInterface = global.CSInterface;
      if (typeof CSInterface === 'function') {
        return (new CSInterface()).openURLInDefaultBrowser(resolved);
      }
    } catch (e1) { /* silent */ }
  }

  function closestDataExternal(node) {
    var el = node;
    while (el && el !== doc && el !== null) {
      try {
        if (el.nodeType === 1 && el.getAttribute) {
          var raw = el.getAttribute('data-external');
          if (raw != null && String(raw).length > 0) return el;
        }
      } catch (e) { /* continue */ }
      el = el.parentElement;
    }
  ...
  function onDataExternalClick(e) {
    var el = null;
    if (e.target && typeof e.target.closest === 'function') {
      el = e.target.closest('[data-external]');
    } else {
      el = closestDataExternal(e.target);
    }
    if (!el) return;
    var raw = el.getAttribute('data-external');
  ...
  function bindExternalDelegation() {
    try {
      doc.addEventListener('click', onDataExternalClick, false);
    } catch (e1) { /* silent */ }
  }

  var FEEDBACK_TEXT = '\u2713 Copied email';
  var FEEDBACK_CLASS = 'email-copy-feedback';
  var VISIBLE_CLASS = 'email-copy-feedback--visible';
  var DISPLAY_MS = 2000;
  var FADE_MS = 200;

  function parseEmailFromMailto(href) {
    if (!href || typeof href !== 'string') return '';
    var trimmed = href.trim();
    if (trimmed.toLowerCase().indexOf('mailto:') !== 0) return '';
    var rest = trimmed.slice(7);
    var q = rest.indexOf('?');
  ...
  function closestHelpLinkStack(el) {
    if (!el) return null;
    if (typeof el.closest === 'function') return el.closest('.help-link-stack');
    var n = el;
    while (n && n !== doc.body) {
      if (n.classList && n.classList.contains('help-link-stack')) return n;
      n = n.parentElement;
    }
    return null;
  }

  function getFeedbackContainer(anchorElement) {
    return closestHelpLinkStack(anchorElement) || anchorElement.parentElement || doc.body;
  }

  function removeExistingFeedback(container) {
    var prev = container.querySelector('.' + FEEDBACK_CLASS);
    if (prev && prev.parentNode) prev.parentNode.removeChild(prev);
  }

  function showEmailCopiedFeedback(anchorElement) {
    var container = getFeedbackContainer(anchorElement);
    removeExistingFeedback(container);

    var el = doc.createElement('span');
    el.className = FEEDBACK_CLASS;
    el.setAttribute('role', 'status');
    el.setAttribute('aria-live', 'polite');
    el.textContent = FEEDBACK_TEXT;
    ...
    global.setTimeout(function () {
      ...
      global.setTimeout(done, FADE_MS + 80);
    }, DISPLAY_MS);
  }

  function copyEmailFromLink(linkElement) {
    if (!linkElement || !linkElement.getAttribute) {
      return Promise.reject(new Error('Invalid link'));
    }
    var email = parseEmailFromMailto(linkElement.getAttribute('href') || '');
    if (!email) return Promise.reject(new Error('No email in href'));
    return copyTextToClipboard(email).then(function () {
      showEmailCopiedFeedback(linkElement);
    });
  }
  ...
  function onHelpLinkClick(e) {
    ...
    e.preventDefault();
    e.stopPropagation();
    copyEmailFromLink(a).catch(function () { /* silent */ });
  }

  function initHelpEmailLinks(root) {
    root = root || doc;
    var list = root.querySelectorAll('a.help-link[href^="mailto:"]');
    for (var i = 0; i < list.length; i++) {
      list[i].addEventListener('click', onHelpLinkClick);
    }
  }
  ...
  onDomReady(function () {
    bindExternalDelegation();
    initHelpEmailLinks(doc);
  });

  global.openExternalUrl = openExternalUrl;
  global.parseEmailFromMailto = parseEmailFromMailto;
  global.copyTextToClipboard = copyTextToClipboard;
  global.showEmailCopiedFeedback = showEmailCopiedFeedback;
  global.copyEmailFromLink = copyEmailFromLink;
  global.initHelpEmailLinks = initHelpEmailLinks;
})(typeof window !== 'undefined' ? window : this);

/* global CSInterface */
/* Helpers panel: UI, CEP bridge, preferences, updates. */
(function () {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCsInterface);
  } else {
    initCsInterface();
  }
  ...
  function pingJSX(callback) {
    window.cs.evalScript('try { if ($.global && typeof $.global.helpersPing === "function") { $.global.helpersPing() } else { "BRIDGE_ERROR: helpersPing not found" } } catch(e) { "BRIDGE_ERROR: " + e.toString(); }', function(res) {
  ...
  function loadJSX(callback) {
    ...
    const extRoot = window.cs.getSystemPath(SystemPath.EXTENSION);
    if (!extRoot) {
    ...
    pingJSX(function(pingResult) {
      if (pingResult && pingResult.indexOf('OK') === 0) {
        ...
        window.cs.evalScript(checkScript, function(res) {
  ...
  function loadHostJSX(hostPath, callback) {
    ...
    window.cs.evalScript(loadScript, function(res2) {
  ...
  function verifyFunction(funcName, callback) {
    window.cs.evalScript('try { ($.global && typeof $.global.' + funcName + ' === "function") ? "OK" : "MISSING" } catch(e) { "ERROR: " + e.toString(); }', function(res) {
  ...
  function evalJsx(script, cb) {
    try {
      ensureJsxLoaded(function () {
        window.cs.evalScript(script, function (result) {
          if (cb) cb(result);
        });
      });
    } catch (e) {
  ...
  /**
   * Show update dialog with message and action to open the download URL in the system browser (CEP).
   */
  function _showUpdateDialog(data) {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = '<div class="update-dialog-box" role="dialog" aria-labelledby="updateDialogTitle" aria-modal="true">' +
        ...
        '<button type="button" id="updateDialogLink" class="update-dialog-link" data-external="">Download update</button>' +
        '</div></div>';
    ...
    if (linkEl) {
      ...
      linkEl.setAttribute('data-external', downloadUrl);
      linkEl.onclick = null;
    }
  ...
  function initializeSettings() {
    ...
    if (checkUpdatesBtn) {
      checkUpdatesBtn.addEventListener('click', function () {
        var pending = _getPendingUpdate();
        if (pending && pending.downloadUrl) {
          if (typeof window.openExternalUrl === 'function') window.openExternalUrl(pending.downloadUrl);
        } else {
  ...
  function getGlobalStrokeWeight() {
    var n = Number(globalStrokeWeight);
    if (!Number.isFinite(n) || n < 0) return DEFAULT_GLOBAL_STROKE;
    return n;
  }
